...
for fichier in os.listdir(dossier_cv):
    if fichier.lower().endswith(".pdf"):
        ...
        # ici tu peux traiter texte comme tu veux, ou appeler extract_info_detaille(texte)
        infos = extract_info_detaille(texte)
        print(f"Infos extraites du fichier {fichier} :")
        print(infos)
...
match = re.search(r"(compétences.*?)(langues|expériences|formations|centres|$)", texte, re.IGNORECASE | re.DOTALL)
if match:
        bloc = match.group(1)
...
match = re.search(r"(langues.*?)(expériences|formations|centres|$)", texte, re.IGNORECASE | re.DOTALL)
if match:
        bloc = match.group(1)
        lignes_bloc = bloc.splitlines()
        for ligne in lignes_bloc:
...
match = re.search(r"(expériences?.*?)(formations|compétences|langues|centres|parcours|$)", texte, re.IGNORECASE | re.DOTALL)
if match:
...
match = re.search(r"(formations?.*?)(centres|$)", texte, re.IGNORECASE | re.DOTALL)
if match:
        bloc = match.group(1).strip()
        lignes = bloc.splitlines()
...
def exporter_vers_xml(infos, chemin_fichier):
    ...
    for mot in infos.get("competences", []):
        SubElement(comps, "MotCle").text = mot if mot else ""

    # Langues
    langs = SubElement(racine, "Langues")
    for lg in infos.get("langues", []):
        l = SubElement(langs, "Langue")
        SubElement(l, "Nom").text = lg.get("langue", "") if isinstance(lg, dict) else ""
        SubElement(l, "Niveau").text = lg.get("niveau", "") if isinstance(lg, dict) else ""

    # Expériences
    exps = SubElement(racine, "Experiences")
    for exp in infos.get("experiences", []):
        e = SubElement(exps, "Experience")
        SubElement(e, "Poste").text = exp.get("poste", "") if isinstance(exp, dict) else ""
        SubElement(e, "Entreprise").text = exp.get("entreprise", "") if isinstance(exp, dict) else ""
        SubElement(e, "Debut").text = exp.get("debut", "") if isinstance(exp, dict) else ""
        SubElement(e, "Fin").text = exp.get("fin", "") if isinstance(exp, dict) else ""
        SubElement(e, "Description").text = exp.get("description", "") if isinstance(exp, dict) else ""

    # Formations
    forms = SubElement(racine, "Formations")
    for f in infos.get("formations", []):
        fo = SubElement(forms, "Formation")
        SubElement(fo, "Diplome").text = f.get("diplome", "") if isinstance(f, dict) else ""
        SubElement(fo, "Etablissement").text = f.get("etablissement", "") if isinstance(f, dict) else ""
        SubElement(fo, "Annee").text = f.get("annee", "") if isinstance(f, dict) else ""
